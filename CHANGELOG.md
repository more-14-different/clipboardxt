...
- **修复资源管理器复制路径反复弹 UAC**：收藏 IPC 改用仅授权当前 Windows 用户 SID 的显式管道 ACL，允许普通权限 AHK 与管理员 ClipboardX 通信；收藏命令也改为独立非提权路径，IPC 失败时不再进入通用 `runas` 流程

### 剪贴板 · 网址标签

- **自动标记合法网址**：既有与新增的单条 `http/https` 文本显示高对比暖色「网址」标签（英文界面为「URL」），与快捷短语及文件跳转「常用」标签区分；标签可用「网址」「URL」或对应拼音/双拼（如小鹤 `whvi`）检索，并覆盖无需重建索引的既有与冷归档历史
